...
export class RestaurantsComponent implements OnInit {
  ...
  readonly filteredRestaurants: Signal<Entries> = computed(() => {
    let restaurants = this.allRestaurants();
    
    // Apply global location filter (navbar)
    if (this.globalLocationFilter().isActive) {
      restaurants = restaurants.filter((restaurant: Entry) => 
        restaurant.location === this.globalLocationFilter().selectedLocation
      );
    }
  ...
  readonly totalPages: Signal<number> = computed(() => 
    Math.ceil(this.filteredRestaurants().length / this.itemsPerPage)
  );
